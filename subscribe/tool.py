...
def http_get_content(url, user_agent=None, retry_times=5) -> str | None:
...
def __http_get_content(url, headers):
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                "http get %s failed, cost time: %s ms, code %s, %s", url, cost_time, response.status_code, response.text
            )
            return None
