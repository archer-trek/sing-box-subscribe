    ...
    if share_link["type"] == "vmess":
        try:
            vmess_info = {
                ...
                "type": "none",
                "host": share_link.get("ws-opts", {}).get("headers", {}).get("Host", "")
        ...
        base_link = base64.b64encode(
            "{server}:{port}:{protocol}:{cipher}:{obfs}:{password}/?obfsparam={obfsparam}&protoparam={protoparam}&remarks={remarks}&group={group}".format(
                **ssr_info
            ).encode("utf-8")
        ).decode("utf-8")
        ...
        if vless_info["type"] == "ws":
            vless_info["path"] = (
                quote(share_link["ws-opts"].get("path", ""), "utf-8")
                if share_link.get("ws-opts")
                else share_link.get("ws-path", "")
            )
            vless_info["host"] = (
                share_link["ws-opts"].get("headers", {}).get("Host", "")
                if share_link.get("ws-opts")
                else share_link.get("ws-headers", {}).get("Host", "")
            )
        ...
        if type(share_link.get("reserved")) is str:
            warp_info["reserved"] = share_link.get("reserved", "")
        else:
            if type(share_link.get("reserved")) is not type(None):
        ...
        if share_link.get("username"):
            if share_link["password"]:
                ...
                base_link = base64.b64encode(
                    "{user}:{password}@{server}:{port}".format(**http_info).encode("utf-8")
                ).decode("utf-8")
        else:
        ...
        if share_link.get("username"):
            if share_link["password"]:
                ...
                base_link = base64.b64encode(
                    "{user}:{password}@{server}:{port}".format(**socks5_info).encode("utf-8")
                ).decode("utf-8")
        else:
