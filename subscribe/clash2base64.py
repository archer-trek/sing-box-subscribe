...
def clash2v2ray(share_link):
    ...
    if share_link['type'] == 'vmess':
        try:
            vmess_info = {
                "v": "2", "ps": share_link['name'].encode('utf-8', 'surrogatepass').decode('utf-8'),
                ...
                "type": "none", "host": share_link.get('ws-opts', {}).get('headers', {}).get('Host', '')
                or share_link.get('ws-headers', {}).get('Host', ''),
        ...
        if share_link.get('plugin'):
            ...
            link = "ss://{base_link}@{server}:{port}{url_link}".format(
                base_link=base_link, url_link=url_link, **ss_info)
        else:
        ...
        ssr_info = {
            ...
            if share_link.get('obfs-param') != None else '', "protoparam": base64.b64encode(
                share_link.get('protocol-param', '').encode('utf-8')).decode('utf-8'),
        ...
        base_link = base64.b64encode(
            "{server}:{port}:{protocol}:{cipher}:{obfs}:{password}/?obfsparam={obfsparam}&protoparam={protoparam}&remarks={remarks}&group={group}".
            format(**ssr_info).encode('utf-8')).decode('utf-8')
        ...
        if trojan_info['type'] == 'grpc':
            ...
            link = "trojan://{password}@{server}:{port}?sni={sni}&allowInsecure={allowInsecure}&type={type}&serviceName={serviceName}&fp={fp}&alpn={alpn}".format(
                **trojan_info)
            ...
            link = "trojan://{password}@{server}:{port}?sni={sni}&allowInsecure={allowInsecure}&type={type}&host={host}&path={path}&fp={fp}&alpn={alpn}".format(
                **trojan_info)
        ...
        if vless_info['type'] == 'ws':
            vless_info["path"] = quote(share_link['ws-opts'].get('path', ''),
                                       'utf-8') if share_link.get('ws-opts') else share_link.get('ws-path', '')
            vless_info["host"] = share_link['ws-opts'].get('headers', {}).get('Host', '') if share_link.get(
                'ws-opts') else share_link.get('ws-headers', {}).get('Host', '')
            link = "vless://{uuid}@{server}:{port}?encryption=none&security={security}&sni={sni}&fp={fp}&type={type}&host={host}&path={path}&flow={flow}&allowInsecure={allowInsecure}".format(
                **vless_info)
            ...
            if share_link.get('reality-opts'):
                ...
                link = "vless://{uuid}@{server}:{port}?encryption=none&security={security}&sni={sni}&type={type}&serviceName={serviceName}&fp={fp}&flow={flow}&allowInsecure={allowInsecure}&pbk={pbk}&sid={sid}".format(
                    **vless_info)
            else:
                link = "vless://{uuid}@{server}:{port}?encryption=none&security={security}&sni={sni}&type={type}&serviceName={serviceName}&fp={fp}&flow={flow}&allowInsecure={allowInsecure}".format(
                    **vless_info)
            ...
            if share_link.get('reality-opts'):
                ...
                vless_info["pbk"] = share_link['reality-opts']['public-key']
                vless_info["sid"] = share_link.get('reality-opts', {}).get('short-id', '')
                link = "vless://{uuid}@{server}:{port}?encryption=none&security={security}&sni={sni}&serverName={sni}&type={type}&fp={fp}&flow={flow}&allowInsecure={allowInsecure}&pbk={pbk}&sid={sid}".format(
                    **vless_info)
            else:
        ...
        if share_link.get('ipv6'):
            ...
            if warp_info.get('reserved'):
                ...
                link = "wg://{server}:{port}?publicKey={publicKey}&privateKey={privateKey}&presharedKey={presharedKey}&ip={ip},{ipv6}&udp=1#{name}".format(
                    **warp_info)
        else:
            if warp_info.get('reserved'):
                link = "wg://{server}:{port}?publicKey={publicKey}&privateKey={privateKey}&presharedKey={presharedKey}&ip={ip}&udp=1&reserved={reserved}#{name}".format(
                    **warp_info)
            else:
                link = "wg://{server}:{port}?publicKey={publicKey}&privateKey={privateKey}&presharedKey={presharedKey}&ip={ip}&udp=1#{name}".format(
                    **warp_info)
        return link
        # TODO
        ...
        if share_link.get('username'):
            if share_link['password']:
                ...
                base_link = base64.b64encode("{user}:{password}@{server}:{port}".format(**
                                             http_info).encode('utf-8')).decode('utf-8')
        else:
        ...
        if share_link.get('username'):
            if share_link['password']:
                ...
                base_link = base64.b64encode("{user}:{password}@{server}:{port}".format(**
                                             socks5_info).encode('utf-8')).decode('utf-8')
        else:
