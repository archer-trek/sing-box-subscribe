...
def parse(data):
    ...
    netquery = dict((k, v if len(v) > 1 else v[0]) for k, v in parse_qs(server_info.query).items())
    node = {
        ...
        "tls": {"enabled": True, "server_name": netquery.get("sni", netquery.get("peer", "")), "insecure": False},
    }
    ...
    if netquery.get("fp"):
        node["tls"]["utls"] = {"enabled": True, "fingerprint": netquery.get("fp")}
